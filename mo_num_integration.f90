! >> sylvia_20200618 make this a module
MODULE mo_num_integration
...
!
  ...
  !real ( kind = 4 ) r1mach ! >> sylvia_20200618, Comment out to not overwrite function.
...
!
  ...
  real ( kind = 8 )  D1, D2, DELTA!, D1MACH ! >> sylvia_20200618, Comment out to not overwrite function.
  integer ( kind = 4 ) ISMON, ITRUE
...
!
  ...
  !real ( kind = 8 )  D1MACH ! >> sylvia_20200618, Comment out to not overwrite function.

  !real ( kind = 4 )  RAND ! >> sylvia_20200618, Comment out to not overwrite function.
  !EXTERNAL  RAND
!
...
!
  ...
  LOGICAL  SKIP !, COMP ! >> sylvia_20200618, Comment out to not overwrite function.
...
!
  ...
  CALL DCHFEV (0.D0, 1.D0, 3.D0, 7.D0, 3.D0, 6.D0, 0, [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -2
  CALL DCHFEV (1.D0, 1.D0, 3.D0, 7.D0, 3.D0, 6.D0, 1, [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -2, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -1
  CALL DCHFDV (0.D0, 1.D0, 3.D0, 7.D0, 3.D0, 6.D0, 0, [DUM], [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -2
  CALL DCHFDV (1.D0, 1.D0, 3.D0, 7.D0, 3.D0, 6.D0, 1, [DUM], [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -2, LOUT, KPRINT) )  NERR = NERR + 1
!
...
!
  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -1
  SKIP = .FALSE.
  CALL DPCHFE (1, X, F, D, 1, SKIP, 0, [DUM], [DUM], IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -3
  SKIP = .FALSE.
  CALL DPCHFE (N, X, F, D, 1, SKIP, 0, [DUM], [DUM], IERR) ! >> sylvia_20200618, Adding [] to DUM
  ...
  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -1
  SKIP = .FALSE.
  CALL DPCHFD (1, X, F, D, 1, SKIP, 0, [DUM], [DUM], [DUM], IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -3
  SKIP = .FALSE.
  CALL DPCHFD (N, X, F, D, 1, SKIP, 0, [DUM], [DUM], [DUM], IERR) ! >> sylvia_20200618, Adding [] to DUM
  IF (.NOT. COMP (IERR, -3, LOUT, KPRINT) )  NERR = NERR + 1
...
!
  ...
  SAVE HALF
  !real ( kind = 8 )  DPCHDF, DPCHST ! >> sylvia_20200618, Comment out to not overwrite function.
!
!  INITIALIZE.
!
  DATA HALF/.5D0/
...
!
  ...
  !real ( kind = 8 )  DPCHST ! >> sylvia_20200618, Comment out to not overwrite function.
  integer ( kind = 4 )  N
...
!
  ...
  real ( kind = 8 )  DELTA
  !integer ( kind = 4 ) DCHFCM ! >> sylvia_20200618, Comment out to not overwrite function.
!
!  CHECK ARGUMENTS.
!
  IF ( .not. skip ) then
...
!
  ...
  !real ( kind = 8 )  DCHFIE, DPCHID  ! >> sylvia_20200618, Comment out to not overwrite function.
...
!
  ...
  !real ( kind = 8 ) DPCHIA, D1MACH ! >> sylvia_20200618, Comment out to not overwrite function.
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  !LOGICAL  COMP ! >> sylvia_20200618, Comment out to not overwrite function.
  !real ( kind = 8 ) D1MACH
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  !real ( kind = 8 ) DBVALU, D1MACH ! >> sylvia_20200618, Comment out to not overwrite function.
  !EXTERNAL  DBVALU, DPCHBS, D1MACH ! >> sylvia_20200618, Comment out to not overwrite function.
...
!
  ...
  real ( kind = 8 )  X(N), F(N), D(N), A, B
  integer ( kind = 4 )  INCFD
  !real ( kind = 8 )  DPCHIA ! >> sylvia_20200618, Scalar->array
  LOGICAL SKIP
...
!
  ...
  SAVE HALF, ONE
  !real ( kind = 8 )  DPCHDF   ! >> sylvia_20200618, Scalar->array
...
!
  ...
  SAVE ONE, FACT
  SAVE THIRD
  !real ( kind = 8 )  D1MACH ! >> sylvia_20200618, Scalar->array
...
!
  ...
  !real ( kind = 4 ) R1MACH   ! >> sylvia_20200618, Scalar->array
  !real ( kind = 4 ) RAND  ! >> sylvia_20200618, Scalar->array
  !EXTERNAL  RAND
!
!  DEFINE RELATIVE ERROR WITH FLOOR.
!
...
!
  ...
  LOGICAL  SKIP !,COMP ! >> sylvia_20200618, Scalar->array
  PARAMETER (N = 10)
...
!
  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -1
  CALL CHFEV (0.E0, 1.E0, 3.E0, 7.E0, 3.E0, 6.E0, 0, [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Scalar->array
  IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -2
  CALL CHFEV (1.E0, 1.E0, 3.E0, 7.E0, 3.E0, 6.E0, 1, [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Scalar->array
  IF (.NOT. COMP (IERR, -2, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -1
  CALL CHFDV (0.E0, 1.E0, 3.E0, 7.E0, 3.E0, 6.E0, 0, [DUM], [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Scalar->array
  IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1

  IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -2
  CALL CHFDV (1.E0, 1.E0, 3.E0, 7.E0, 3.E0, 6.E0, 1, [DUM], [DUM], [DUM], NEXT, IERR) ! >> sylvia_20200618, Scalar->array
  IF (.NOT. COMP (IERR, -2, LOUT, KPRINT) )  NERR = NERR + 1
!
!  SET UP PCH DEFINITION.
!
...
!
      IF (KPRINT .GE. 3)  WRITE (LOUT, 5001)  -1
      SKIP = .FALSE.
      CALL PCHFE (1, X, F, D, 1, SKIP, 0, [DUM], [DUM], IERR)  ! >> sylvia_20200618, scalar->array here and below
      IF (.NOT. COMP (IERR, -1, LOUT, KPRINT) )  NERR = NERR + 1
...
!
  ...
  SAVE HALF
  !real ( kind = 4 ) PCHDF, PCHST ! >> sylvia_20200618, Scalar->array
!
!  INITIALIZE.
!
  DATA  HALF /0.5/
...
!
  ...
  real ( kind = 4 ) W1, W2
  !real ( kind = 4 ) PCHST ! >> sylvia_20200618, Scalar->array
...
!
  ...
  integer ( kind = 4 )  I, NSEG
  real ( kind = 4 ) DELTA
  !integer ( kind = 4 )  CHFCM ! >> sylvia_20200618, Scalar->array
!
!  CHECK ARGUMENTS.
!
  IF ( .not. SKIP ) then
...
!
  ...
  !real ( kind = 4 ) PCHID, CHFIE >> sylvia_20200618, Commenting out to not overwrite function.
...
!
  ...
  !real ( kind = 4 ) PCHST >> sylvia_20200618, Commenting out to not overwrite function.
!
...
!
  ...
  integer ( kind = 4 )  INCFD
  !real ( kind = 4 ) PCHIA >> sylvia_20200618, Commenting out to not overwrite function.
  LOGICAL SKIP
...
!
  ...
  LOGICAL  FAIL, SKIP
  !real ( kind = 4 ) PCHIA, R1MACH >> sylvia_20200618, Commenting out to not overwrite function.
!
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  !LOGICAL  COMP >> sylvia_20200618, Commenting out to not overwrite function.
  !real ( kind = 4 ) R1MACH >> sylvia_20200618, Commenting out to not overwrite function.
  integer ( kind = 4 ) I, IC(2), IERR, IFAIL, N, NBAD, NBADZ, NWK
  PARAMETER  (N = 9,  NWK = 2*N)
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  !real ( kind = 4 ) BVALU, R1MACH >> sylvia_20200618, Commenting out to not overwrite function.
  !EXTERNAL  BVALU, PCHBS, R1MACH >> sylvia_20200618, Commenting out to not overwrite function.
  integer ( kind = 4 ) I, IERR, IFAIL, INBV, J, KNOTYP, K, N, NDIM, NKNOTS
  PARAMETER  (N = 9)
...
!
  ...
  SAVE HALF, ONE
  !real ( kind = 4 ) PCHDF >> sylvia_20200618, Commenting out to not overwrite function.
...
!
  ...
  integer ( kind = 4 ) i
  !integer ( kind = 4 ) j4save >> sylvia_20200618, Commenting out to not overwrite function.
...
!
  ...
  CHARACTER*148 CBUFF
  integer ( kind = 4 ) i
  !integer ( kind = 4 ) i1mach >> sylvia_20200618, Commenting out to not overwrite function.
...
!
  ...
  !integer ( kind = 4 ) j4save, >> sylvia_20200618, Comment out to not overwrite function.
  integer ( kind = 4 ) kontrl
...
!
  ...
  !integer ( kind = 4 ) J4SAVE   ! >> sylvia_20200618, Commeting out to not overwrite function.
  integer ( kind = 4 ) junk
  integer ( kind = 4 ) kontrl
...
! >> sylvia_20200617
! Looking here for numerical integration in Fortran:
...
FUNCTION INTEGRATE(x, y, a, b)
     ...
     REAL(wp), INTENT(IN) :: x(:)         ! Input x
     REAL(wp), INTENT(IN) :: y(SIZE(x))   ! Output y(x)
     REAL(wp), INTENT(IN) :: a, b         ! Left and right endpoints

     REAL(wp)             :: d(SIZE(x))
     REAL(wp)             :: INTEGRATE    ! Integral ∫y(x)·dx
     INTEGER              :: err

     ! Create a piecewise cubic Hermitian interpolation of the input data.
     CALL dpchez(SIZE(x), x, y, d, .FALSE., [0.d0], 0, err)

     ! Integrate the interpolation in the provided range
     INTEGRATE = dpchqa(SIZE(x), x, y, d, a, b, err)  ! >> sylvia_20200618, Changed r to INTEGRATE.

     RETURN  ! >> sylvia_20200618, Added this on my own.
END FUNCTION
! << sylvia_20200617

! >> sylvia_20200622
! The Hermitian polynomials above are giving weird results and trapezoid
! precision is probably sufficient in this case.
FUNCTION TRAPEZOID_RULE(x, y)

    IMPLICIT NONE
    ! Calculate the integral of an array Y with respect to X using the trapezoid
    ...
    REAL(wp), INTENT(IN) :: x(:)              ! Input x
    REAL(wp), INTENT(IN) :: y(size(x))        ! Output y(x)
    REAL(wp)             :: TRAPEZOID_RULE    ! Integral ∫y(x)·dx

    ASSOCIATE(n => SIZE(x))
      TRAPEZOID_RULE = SUM((y(1+1:n-0) + y(1+0:n-1)*x(1+1:n-0) - x(1+0:n-1)))/2
    END ASSOCIATE
   
    RETURN
END FUNCTION
FUNCTION TRAPEZOID_RULE2(x, y)
    ...
    REAL(wp), INTENT(IN) :: x(:)              ! Input x
    REAL(wp), INTENT(IN) :: y(size(x))        ! Output y(x)
    REAL(wp)             :: TRAPEZOID_RULE2   ! Integral ∫y(x)·dx

    TRAPEZOID_RULE2 = 0._wp
    DO n = 2, SIZE(x)
       TRAPEZOID_RULE2 = TRAPEZOID_RULE2 + (x(n) - x(n-1))*(y(n) + y(n-1))/2._wp
    END DO
